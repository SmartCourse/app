...
Promise.all([
    ...
    .then(([qIDs, rIDs]) => {
        ...
        data = sqlLikes(qIDs.map(({ id, userID }) => ({ objectType: TABLE_NAMES.QUESTIONS, objectID: id, userID })))
        data += sqlLikes(qIDs.map(({ id, userID }) => ({ objectType: TABLE_NAMES.COMMENTS, objectID: id, userID })))
        data += sqlLikes(rIDs.map(({ id, userID }) => ({ objectType: TABLE_NAMES.REVIEWS, objectID: id, userID })))
        data += sqlLikes(rIDs.map(({ id, userID }) => ({ objectType: TABLE_NAMES.COMMENTS, objectID: id, userID })))
